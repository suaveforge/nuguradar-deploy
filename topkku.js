(()=>{
  ...
  let theme='lavender',photo=null,elements=[],selected=-1,drag=null,history=[],sourceMeta=null;
  ...
  function statusText(){if(selected>=0)return'선택됨 · 드래그해서 옮기고 아래 버튼으로 조절하세요';if(photo&&sourceMeta?.source==='watch'){const who=sourceMeta.artist?`${sourceMeta.artist} · `:'';const when=Number.isFinite(Number(sourceMeta.time))?`${timeLabel(sourceMeta.time)} 장면`: '영상 장면';return`${who}${when}을 가져왔어요 ✨ 스티커나 문구를 얹어보세요`}if(photo)return'스티커나 문구를 추가해보세요';return'사진을 먼저 골라주세요'}
  ...
  function loadPhotoData(src,meta=null){if(!src)return;const img=new Image();img.onload=()=>{photo=img;sourceMeta=meta;selected=-1;draw()};img.onerror=()=>{sourceMeta=null;draw()};img.src=src}
  ...
  $('#resetBtn').onclick=()=>{if(!photo&&!elements.length&&theme==='lavender')return;saveHistory();photo=null;sourceMeta=null;elements=[];selected=-1;theme='lavender';$$('[data-theme]').forEach(b=>b.classList.toggle('active',b.dataset.theme==='lavender'));$('#photoInput').value='';draw()};
